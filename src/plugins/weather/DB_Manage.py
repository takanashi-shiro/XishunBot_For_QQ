...
def find_bd(qq_number):
    sql = "select * from weathers where qq_number = '%s'" % qq_number
    con = link()
    cursor = con.cursor(buffered=True)
    cursor.execute(sql)
...
def insert(qq_number, city):
    if find_bd(qq_number):
        res = '该qq已经绑定了一个城市！'
        return res
    sql = "insert into weathers values ('%s','%s')" % (qq_number, city)
    con = link()
    cursor = con.cursor(buffered=True)
    try:
        cursor.execute(sql)
...
def update(qq_number, city):
    sql = "update weathers set citys = '%s' where qq_number = '%s' " % (city, qq_number)
    con = link()
    cursor = con.cursor(buffered=True)
    try:
        cursor.execute(sql)
...
def get_city(qq_number):
    sql = "select citys from weathers where qq_number = '%s'" % qq_number
    con = link()
    cursor = con.cursor(buffered=True)
    try:
        cursor.execute(sql)
...
def unband(qq_number):
    sql = "delete from weathers where qq_number = '%s'" % qq_number
    con = link()
    cursor = con.cursor(buffered=True)
    try:
        cursor.execute(sql)
